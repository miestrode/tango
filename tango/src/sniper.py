        ...
        if drop_time_response.status != 200:
            ...
            async with SESSION.session.get(f"https://account.mojang.com/available/minecraft/{username}") as availability_response:
                if await availability_response.text() == "TAKEN":
                    tango.src.error.UsernameUnavailableError(await availability_response.text())
    ...
    async def __authenticate(self) -> None:
        ...
        async with SESSION.session.post("https://authserver.mojang.com/authenticate", headers={"Content-Type": "application/json"},
            ...
            if authentication_response.status != 200:
                tango.src.error.InvalidCredentialsError(await authentication_response.text())

            print(f"{tango.src.error.LIGHT_BLUE}Email and password confirmed, {self.email} exists.")
            ...
            async with SESSION.session.get("https://api.mojang.com/user/security/location", headers=self.authorization_header) as security_response:
                if security_response.status != 204:
                    ...
                    async with SESSION.session.get("https://api.mojang.com/user/security/challenges", headers=self.authorization_header) as question_response:
                        ...
                        print(f"{tango.src.error.LIGHT_BLUE}In order to finish authenticating {self.email} you need to answer 3 security questions. Please do so.")

                        # Retrieve answers supplied by the user
                        answers = [{"id": key, "answer": input(f"\n{tango.src.error.GRAY}{questions[key]}\n{tango.src.error.DARK_GRAY}> ")} for key in questions]
                        ...
                        async with SESSION.session.post("https://api.mojang.com/user/security/location", headers=self.authorization_header, json=answers) as answer_response:
                            if answer_response.status != 204:
                                tango.src.error.SecurityAnswerError(await answer_response.text())
    ...
    async def send_snipe_request(self, target_name: str) -> datetime.datetime:
        ...
        async with SESSION.session.put(f"https://api.minecraftservices.com/minecraft/profile/name/{target_name}", headers=self.authorization_header) as name_change_response:
            if name_change_response.status == 200:
                ...
                self.got_name = True
                print(f"{tango.src.error.BLUE}Succeeded{tango.src.error.GRAY} in sniping the name {target_name} to {self.email} at {datetime.datetime.now()}.")
            else:
                print(f"{tango.src.error.RED}Failed{tango.src.error.GRAY} in sniping the name {target_name} to {self.email} at {datetime.datetime.now()}.")
    ...
    async def send_snipe_request(self, target_name: str) -> datetime.datetime:
        ...
        async with SESSION.session.post(f"https://api.minecraftservices.com/minecraft/profile", headers=self.authorization_header,
            ...
            if claim_response.status == 200:
                asyncio.get_event_loop().close()
                self.got_name = True
                print(f"{tango.src.error.BLUE}Succeeded{tango.src.error.GRAY} in claiming the name {target_name} to {self.email} at {datetime.datetime.now()}.")
            else:
                print(f"{tango.src.error.RED}Failed{tango.src.error.GRAY} in claiming the name {target_name} to {self.email} at {datetime.datetime.now()}.")
...
class Session:
    ...
    def run(self) -> None:
        ...
        print(f"{tango.src.error.GRAY}\nSniping session began at {tango.src.error.BLUE}{current_time.strftime('%Y/%m/%d %H:%M:%S')}{tango.src.error.GRAY}.")
        print(f"{tango.src.error.GRAY}Attempts to snipe {tango.src.error.BLUE}{self.target_name}{tango.src.error.GRAY} will begin at {tango.src.error.BLUE}{start_time.strftime('%Y/%m/%d %H:%M:%S')}{tango.src.error.GRAY}.")

        asyncio.run(asyncio.sleep(sleep_time))
        print(f"{tango.src.error.GRAY}Sniping started.")
