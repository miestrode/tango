...
def gather_info(prompt: str, requested_type: typing.Callable) -> any:
    ...
    response = input(f"{tango.src.error.GRAY}{prompt}{tango.src.error.BLUE}{tango.src.error.BOLD}\n> ")

    try:
        return requested_type(response)
    except ValueError as exception:
        tango.src.error.InputTypeError(f"Input is not of type {requested_type.__name__}", exception.__str__())
...
def build_config() -> None:
    ...
    """
    # Display the logo
    print(f"""{tango.src.error.BLUE}{tango.src.error.BOLD}
88888888888                                  
    888                                      
    888                                      
...
{tango.src.error.GRAY}{tango.src.error.BOLD}An open-source name sniper      {tango.src.error.BLUE}{tango.src.error.BOLD}888          
                           Y8b d88P          
                            "Y88P" """)

print(f"""
{tango.src.error.LIGHT_BLUE}Welcome to the json builder. This program will create a configuration file for you.

Note that:{tango.src.error.GRAY}

{tango.src.error.LIGHT_BLUE}*{tango.src.error.GRAY} Your sniping offset is gradually changed every snipe to better adapt to your computer.
Using this program will overwrite the current offset value.

{tango.src.error.LIGHT_BLUE}*{tango.src.error.GRAY} You should make sure no one is watching you.
You are about to enter {tango.src.error.BOLD}important{tango.src.error.GRAY} credentials.

{tango.src.error.LIGHT_BLUE}*{tango.src.error.GRAY} Some configuration settings are not filled out here, and are set to default values.
...
for index in range(account_count):
    ...
    if accounts.get(email, None):
        tango.src.error.ShadowingError("Email already used.", email)
...
with open("../tango/src/config.json", 'w') as file:
